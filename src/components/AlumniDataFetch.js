...
const AlumniTable = () => {
  ...
  const fetchAlumniData = async () => {
    try {
      ...
      const simplifiedData = filteredData.map(
        ({ name, rank, designation, location }) => ({
          name,
          rank,
          designation,
          location,
  ...
  return (
    <div className="text-center">
      <h2 className="text-3xl font-bold mb-4">
        Meet Our Proud Alumni of NCC, NITJ
      </h2>
      <p className="text-gray-700 mb-8">
        Explore the achievements and success stories of our talented alumni who
        ...
        <tbody>
          {alumniData.map((alumni, index) => (
            <tr key={index} className={index % 2 === 0 ? "bg-gray-100" : ""}>
              ...
              <td className="py-2 px-4 border">{alumni.rank}</td>
